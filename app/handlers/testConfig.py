...
class TestConfigFeatureThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            subprocess.check_output(["feature-based-tracking", tracking_path, "--tf", "--database-filename", db_path], stderr=subprocess.STDOUT)
            subprocess.check_output(["display-trajectories.py", "-i", get_project_video_path(self.identifier), "-d", db_path, "-o", project_path + "/homography/homography.txt", "-t", "feature", "--save-images", "-f", str(self.frame_start), "--last-frame", str(self.frame_start + self.num_frames), "--output-directory", images_folder], stderr=subprocess.STDOUT)
...
class TestConfigObjectThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            subprocess.check_output(["feature-based-tracking",tracking_path,"--gf","--database-filename",obj_db_path], stderr=subprocess.STDOUT)
            subprocess.check_output(["classify-objects.py", "--cfg", tracking_path, "-d", obj_db_path], stderr=subprocess.STDOUT)  # Classify road users
            subprocess.check_output(["display-trajectories.py", "-i", get_project_video_path(self.identifier),"-d", obj_db_path, "-o", project_path + "/homography/homography.txt", "-t", "object", "--save-images", "-f", str(self.frame_start), "--last-frame", str(self.frame_start + self.num_frames), "--output-directory", images_folder], stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as err_msg:
